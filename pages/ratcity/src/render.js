...
const Render = (() => {
  ...
  const ART_IDS = {
    ...
    rat_boss: { mode: 'up', w: 0.95 },   // frontal art: stands and looms, never rotates
  ...
  function applyArt(group, id, keeps, scl) {
    ...
    const m = new THREE.Mesh(geo2, new THREE.MeshBasicMaterial({ map: a.tex, transparent: true, alphaTest: 0.3, depthWrite: false }));
    if (spec.mode === 'flat') { m.rotation.x = -Math.PI / 2; m.rotation.z = Math.PI; m.position.y = 0.045; }
    else { geo2.translate(0, h / 2, 0); m.position.y = 0.02; m.position.z = 0.01; }
    if (scl) m.scale.setScalar(scl);
    const keep = new Set(keeps || []);
    for (const c of group.children) if (!keep.has(c)) c.visible = false;
    group.add(m);
    u.artDone = true; u.artMesh = m; u.artMode = spec.mode;
    return true;
  }
  ...
  function buildBoard() {
    ...
    C.SLOTS.forEach(s => {
      ...
      if (s.id === 'stoopRail') {
        ...
        lamp = new THREE.PointLight(col(P.bagYellow), 0, 7); at(lamp, s.x - 0.45, 3.2, s.z - 0.1); scene.add(lamp);
  ...
  function addTag(count) {
    for (let i = 0; i < count; i++) {
      ...
      const t = art.tag
        ? new THREE.Mesh(new THREE.PlaneGeometry(w * 1.6, w * 1.6 * art.tag.aspect), new THREE.MeshBasicMaterial({ map: art.tag.tex, transparent: true, alphaTest: 0.1, depthWrite: false }))
  ...
  function syncTowers(g, dt) {
    for (const s of g.slots) {
      ...
      if (v.prop) v.prop.visible = !u.artDone;
  ...
  function syncRats(g, dt) {
    ...
    for (const r of g.rats) {
      ...
      m.rotation.y = u.artMode === 'up' ? 0 : r.dir;
      if (u.artMode === 'up') u.bags.forEach((b, i) => b.position.set((i - 1) * 0.3, 0.5 + (i % 2) * 0.22, 0.32));
  ...
  function syncFx(dt) {
    ...
    if (sliceFx) {
      ...
      if (!sliceFx.mesh) {
        if (art.pizza_slice) { const a = art.pizza_slice, w = ART_IDS.pizza_slice.w; sliceFx.mesh = new THREE.Mesh(new THREE.PlaneGeometry(w, w * a.aspect), new THREE.MeshBasicMaterial({ map: a.tex, transparent: true, alphaTest: 0.3, depthWrite: false })); }
  ...
  function syncArtStatics() {
    if (!facadeArt && art.facade) {
      ...
      facadeArt = new THREE.Mesh(geo2, new THREE.MeshBasicMaterial({ map: a.tex, transparent: true, alphaTest: 0.1, depthWrite: false }));
